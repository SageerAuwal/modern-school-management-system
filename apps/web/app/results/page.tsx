...
export default function ResultsPage() {
    ...
    async function loadMeta() {
      try {
        const [clsRes, trmRes] = await Promise.all([
          ...
          fetch(`${API}/api/v1/terms`, { credentials: "include" }).then((r) => r.json()),
        ]);

        if (Array.isArray(clsRes)) setClasses(clsRes);
        if (Array.isArray(trmRes)) {
          setTerms(trmRes);
          const current = trmRes.find((t: Term) => t.isCurrent);
          if (current) setSelectedTermId(current.id);
          else if (trmRes.length > 0) setSelectedTermId(trmRes[0].id);
        }
      } catch {
  ...
  async function handleCheckResult(e?: React.FormEvent) {
    if (e) e.preventDefault();
    if (!selectedStudentId || !selectedTermId || !selectedClassId) {
      setError("Please select a class, term, and student to generate results.");
      return;
    }
    ...
    try {
      const res = await fetch(
        `${API}/api/v1/scores/report-card/${selectedStudentId}?termId=${selectedTermId}&classSectionId=${selectedClassId}`,
        { credentials: "include" }
      );
      ...
      setError("Unable to generate results. Check connection.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="page">
      ...
      <div className="page-header no-print">
        <div>
          <h1 className="page-title">Online Results &amp; Report Cards</h1>
          <p className="page-subtitle">Generate and inspect official student terminal academic reports.</p>
        </div>
        {reportCard && (
          <button
            type="button"
      ...
      <div className="card no-print" style={{ marginBottom: 24 }}>
              ...
              >
                ...
                {students.map((s) => (
                  <option key={s.id} value={s.id}>
                    {s.lastName}, {s.firstName} {s.admissionNumber ? `(${s.admissionNumber})` : ""}
                  </option>
                ))}
              </select>
            </div>
        ...
        {error && (
          <div style={{ marginTop: 16, padding: "10px 14px", borderRadius: "var(--radius-control)", backgroundColor: "var(--color-danger-bg)", color: "var(--color-danger-text)", fontSize: 13 }}>
            {error}
          </div>
        )}
      </div>
        ...
        >
          ...
          <div style={{ textAlign: "center", borderBottom: "2px solid var(--color-ink)", paddingBottom: 18, marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 12, color: "var(--color-text-secondary)", margin: "0 0 6px" }}>
              1 School Road, Kano Municipal, Kano State • Official Terminal Report
            </p>
            <div
              style={{
      ...
      {!reportCard && !loading && (
        <div className="card empty-state">
          ...
          <div className="empty-state-title">Select student criteria above</div>
